...
public class Bosco : Pathfinding {
	...
	private GameObject[] WPList;
	private GameObject bosco;
	...
	void Start () 
	{
		pct = Random.Range (0, 6);
		AIList = GameObject.FindGameObjectsWithTag("Bosco");
		bosco = GameObject.FindGameObjectWithTag ("Bosco");
		WPList = GameObject.FindGameObjectsWithTag ("wp");
		bosco.transform.position = WPList [pct].transform.position;
	}
	...
	private void MoveMethod()
	{
		if (Path.Count > 0)
		{
			...
			transform.position = Vector3.MoveTowards(transform.position, transform.position + direction, Time.deltaTime * 16F);
	...
	void OnTriggerEnter(Collider other){
		...
		if (other.gameObject.CompareTag ("WinPlate")) {
			active = true;
		}
	}
}
